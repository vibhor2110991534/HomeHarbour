...
export default function EditListing() {
  ...
  useEffect(() => {
    if (listing && listing.userRef !== "add user name here"
  ...
  async function onSubmit(e) {
    ...
    if (geolocationEnabled) {
      const response = await fetch(
        `https://geocode.maps.co/search?q=${address.toLowerCase()}`
      );
    ...
    // upadate this updated form datra in api
    ...
    // ).catch((error) => {
    //   setLoading(false);
    //   toast.error("ImgUrls not uploaded");
    //   return;
    // });
    // const formDataCopy = {
    //   ...formData,
      ...
      >
        ...
        <div className="flex items-center justify-center w-full flex-row gap-4">
          <button
            ...
            transition duration-150 ease-in-out w-full ${type === "rent"
                ? "bg-white text-black"
          ...
          <button
            ...
            font-medium text-sm uppercase shadow-md rounded 
            hover:shadow-md focus:shadow-lg active:shadow-lg 
            transition duration-150 ease-in-out w-full ${type === "sale"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
              }`}
            type="button"
